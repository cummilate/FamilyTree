...
public class FamilyApplicationLauncher {
    ...
    public void launch() {
        ...
        InputStream is = FamilyApplicationLauncher.class.getClassLoader()
                .getResourceAsStream("default-family-members.txt");
    ...
    private void acceptAndProcess(Scanner sc, FamilyRelationManager familyRelationManager) {
        String input;
        while (sc.hasNextLine() && !(input = sc.nextLine()).equalsIgnoreCase("EXIT")) {
            try {
